...
To build the docs you must have all optional dependencies for porise
...
class DocBuilder:
    ...
    def __init__(
        ...
        if single_doc and single_doc.endswith(".rst"):
            ...
            self.single_doc_html = f"reference/api/porise.{single_doc}.html"

    def _process_single_doc(self, single_doc):
        """
        Make sure the provided value for --single is a path to an existing
        .rst/.ipynb file, or a porise object that can be imported.

        For example, categorial.rst or porise.DataFrame.head. For the latter,
        return the corresponding file path
        (e.g. reference/api/porise.DataFrame.head.rst).
        """
    ...
    def html(self):
        ...
        zip_fname = os.path.join(BUILD_PATH, "html", "porise.zip")
    ...
    def latex(self, force=False):
        ...
        if sys.platform == "win32":
            ...
            if force:
                for i in range(3):
                    self._run_os("pdflatex", "-interaction=nonstopmode", "porise.tex")
                raise SystemExit(
                    "You should check the file "
                    '"build/latex/porise.pdf" for problems.'
                )
            else:
                self._run_os("make")
            return ret_code
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description="porise documentation builder", epilog=f"Commands: {joined}"
    )
    ...
    argparser.add_argument(
        ...
        help=(
            ...
            "'ecosystem.rst', 'porise.DataFrame.join'"
        ),
    )
    argparser.add_argument(
    ...
    globals()["porise"] = importlib.import_module("porise")
